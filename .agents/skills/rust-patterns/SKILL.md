...
- Using `String` when `&str` suffices in function signatures.
- Using `Box<dyn Error>` in libraries (use `thiserror` instead).
- Ignoring `must_use` warnings on `Result`.
- Blocking the async runtime with long-running sync operations.
